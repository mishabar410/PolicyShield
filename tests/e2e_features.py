...
test("2a. Detect PII in write args",
...
if has_modified:
    ...
    test("2b. PII actually masked in modified_args",
...
test("2c. No PII detected in clean write",
...
test("3a. Write .env triggers APPROVE",
...
if has_approval:
    ...
    test("3b. Approval listed in pending",
         found,
         f"pending_count={len(pending.get('approvals', []))}")

    # 3c: Deny approval
    deny = api("POST", "/respond-approval", {"approval_id": aid, "approved": False, "responder": "test"})
    test("3c. Deny approval",
         deny.get("status") == "ok" or deny.get("approval_id") == aid,
         f"response={deny}")
...
test("4a. First 4 reads allowed (count not yet > 3 at eval time)",
     r.get("verdict") == "ALLOW",
     f"verdict after 4 reads={r.get('verdict')}")

r = check("read", {"path": "file5.txt"}, session)
test("4b. 5th read blocked (count=4 > 3 at eval time)",
     r.get("verdict") == "BLOCK",
...
test("5a. Kill switch activated",
     kill_r.get("status") == "killed" or "kill" in str(kill_r).lower(),
     f"response={kill_r}")

r = check("read", {"path": "hello.txt"}, "s5")
test("5b. All calls blocked after kill",
     r.get("verdict") == "BLOCK",
     f"verdict={r.get('verdict')} msg={r.get('message', '')[:60]}")

resume_r = api("POST", "/resume")
test("5c. Resume from kill switch",
     resume_r.get("status") == "resumed" or "resum" in str(resume_r).lower(),
     f"response={resume_r}")

r = check("read", {"path": "hello.txt"}, "s5b")
test("5d. Normal operation after resume",
     r.get("verdict") == "ALLOW",
     f"verdict={r.get('verdict')}")
...
test("7a. PII detected in tool output",
     pii_found,
     f"pii_types={post_r.get('pii_types')}")

post_r2 = api("POST", "/post-check", {
    "tool_name": "read",
    "args": {"path": "readme.txt"},
    "result": "This is a normal readme file with no sensitive data.",
    "session_id": "s7",
})
test("7b. No PII in clean output",
     len(post_r2.get("pii_types", [])) == 0,
     f"pii_types={post_r2.get('pii_types')}")

# ── Feature 8: CONSTRAINTS INJECTION ─────────────────────
print("\n📋 Feature 8: CONSTRAINTS INJECTION")
constraints_r = api("GET", "/constraints")
summary = constraints_r.get("summary", "")
test("8a. Constraints endpoint returns rules",
     len(summary) > 50 and "BLOCK" in summary,
     f"summary_len={len(summary)}")
test("8b. All rule types present in constraints",
